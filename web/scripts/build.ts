...
import { STATIC_SEARCH_PREFIX, STATIC_TREE_PATH, STATIC_TREE_FLAT_PATH, STATIC_WIKI_PREFIX, RUNTIME_MODE } from "../src/lib/config";
import { WIKI_DIR, EXCLUDE_DIRS, BLOCK_ANALYTICS } from "../src/lib/wiki";
...
async function main() {
  const mode = RUNTIME_MODE || "server";
  const declared = {
  ...
  logger.info({ env, effective: { mode, wikiDir: WIKI_DIR, exclude: EXCLUDE_DIRS, blockAnalytics: BLOCK_ANALYTICS } }, "build:env");
  if (mode === "serverless") {
    ...
    await buildWiki({ wikiDir: WIKI_DIR, outDir: outWiki, sanitize, strategy: "link" });
  } else {
    logger.info("Skip static build steps in server mode");
  }
}
